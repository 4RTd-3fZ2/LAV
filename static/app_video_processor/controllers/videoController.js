import {obtain_date, createCode, put_name_files} from '../utils/helperUtils.js';
import {capture_second, create_elemet_capture} from './imagenControllers.js';

import {consult_guias, datosNumericos, movedatatolav} from '../services/apiService.js';
...
// NOTE : PARA MOVER LOS VIDEO A LAV
export async function openDialog_MovToLAV() {
    ...
    document.getElementById('selectStacks').addEventListener('change', function() {
        actualizarNumeros();
    });
    document.getElementById('btn_mover').addEventListener('click', function() {
        move_files();
    });
...
async function actualizarNumeros() {
    ...
    let selectionStack = document.getElementById('selectStacks').value;
    let totalVideo = document.getElementById('total_videos').value;

    let totalVideos = await  datosNumericos(selectCategory, selectionStack);
    let espacio = 144 - totalVideos;

    document.getElementById('info_lav').innerHTML = `<p><span>${selectCategory}video/Stack_${selectionStack}: </span>${totalVideos}. <span>Espacio: </span>${espacio}</p>\n<p><span>Total videos a mover: </span> ${totalVideo}</p>`
}

async function move_files() {
    let selectCategory = document.getElementById('selectCategory').value;
    let selectionStack = document.getElementById('selectStacks').value;
    let jsonVideos = document.getElementById('insert_code').innerText

    let result = await movedatatolav(selectCategory, selectionStack, jsonVideos);
    console.log(result);
}